...
function ReviewCard({ provider: p, onApprove, onReject }) {
  ...
  return (
    <div style={{
      background: 'var(--k-card)', border: '1.5px solid rgba(245,158,11,0.3)',
      borderRadius: 18, overflow: 'hidden',
      boxShadow: '0 2px 12px rgba(245,158,11,0.08)',
    }}>
      {/* Header strip */}
      <div style={{ background: 'rgba(245,158,11,0.08)', padding: '12px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(245,158,11,0.15)' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {info.specializations.map(s => (
                        <span key={s} style={{ fontSize: 12, padding: '3px 10px', borderRadius: 20, background: 'rgba(5,150,105,0.1)', color: '#059669', fontWeight: 600 }}>{s}</span>
                      ))}
                    </div>
                  </div>
                )}
        ...
        {!showReject && (
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={handleApprove} disabled={busy}
              style={{ flex: 2, padding: '11px', borderRadius: 12, border: 'none', background: busy ? 'var(--k-border)' : '#059669', color: busy ? 'var(--k-muted)' : '#fff', fontWeight: 700, fontSize: 13, cursor: busy ? 'default' : 'pointer' }}>
